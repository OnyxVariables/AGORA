<?php

namespace App\Http\Controllers;

use App\Models\User;
use Illuminate\Support\Facades\Auth;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class CertAuthController extends Controller
{
    public function login(Request $request): JsonResponse
    {
        $resolved = $this->resolveDniFromRequest($request);

        if (isset($resolved['error'])) {
            return $resolved['error'];
        }

        $dni = strtoupper(trim((string) $resolved['dni']));

        $user = User::whereRaw('UPPER(dni) = ?', [strtoupper($dni)])
            ->where('isActive', 1)
            ->first();

        if (!$user) {
            Log::notice('CertAuth: usuario no encontrado o inactivo', ['dni' => $dni]);
            return response()->json(['error' => 'Usuario no autorizado'], 403);
        }

        // sesion Laravel
        Auth::login($user);

        Log::info('CertAuth: login OK', ['dni' => $dni, 'user_id' => $user->id ?? null]);

        // Esto va para react
        return response()->json([
            'roleId' => $user->roleId,
            'dni'    => $user->dni,
        ]);
    }

    /**
     * Certificado cliente (cabeceras SSL) o cabecera X-Demo-DNI solo en local
     * con AGORA_DEMO_AUTH=true.
     *
     * @return array{dni?: string, error?: JsonResponse}
     */
    private function resolveDniFromRequest(Request $request): array
    {
        if (config('agora.demo_auth') && app()->environment('local')) {
            $headerDni = $request->header('X-Demo-DNI');
            if ($headerDni !== null && $headerDni !== '') {
                Log::info('CertAuth: login demo local', ['dni' => $headerDni]);

                return ['dni' => $headerDni];
            }
        }

        $sslVerified = $request->server('SSL_CLIENT_VERIFY') ?? $request->header('X-SSL-Verified');
        $dn = $request->server('SSL_CLIENT_S_DN') ?? $request->header('X-SSL-Client-S-DN');

        Log::info('CertAuth: incoming', ['SSL_CLIENT_VERIFY' => $sslVerified, 'SSL_CLIENT_S_DN' => $dn]);

        if (!$sslVerified || $sslVerified !== 'SUCCESS') {
            return ['error' => response()->json(['error' => 'Certificado no válido'], 401)];
        }

        if (empty($dn)) {
            return ['error' => response()->json(['error' => 'No se pudo leer el certificado'], 401)];
        }

        $dni = null;
        if (preg_match('/serialNumber=IDCES-([0-9]{7,8}[A-Za-z])/', $dn, $matches)) {
            $dni = $matches[1];
        } elseif (preg_match('/serialNumber=([^,\/]+)/', $dn, $matches)) {
            $dni = $matches[1];
        }

        if (!$dni) {
            Log::warning('CertAuth: DNI no extraído', ['dn' => $dn]);

            return ['error' => response()->json(['error' => 'DNI no encontrado'], 401)];
        }

        return ['dni' => $dni];
    }

    public function logout(Request $request)
    {
        Auth::guard('web')->logout();

        $request->session()->invalidate();

        $request->session()->regenerateToken();

        return response()->json(['message' => 'Sesión cerrada'], 200);
    }
}